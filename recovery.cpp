...
prepend_title(const char** headers) {
    ...
    char* header1 = strcat(title1, DataManager::GetStrValue(TW_VERSION_VAR));
    char* title[] = { header1,
...
sdcard_directory(const char* path) {
	...
	if (DataManager::GetIntValue(TW_SORT_FILES_BY_DATE_VAR)) {
    ...
    do {
        ...
        if (chosen_item == 0) {          // item 0 is always "../"
        	...
        	if (get_new_zip_dir > 0)
        	{
        		DataManager::SetStrValue(TW_ZIP_LOCATION_VAR, (char*) path);
                return 1;
        	} else {
            	dec_menu_loc();
                result = -1;
...
int install_zip_package(const char* zip_path_filename) {
	...
	bool md5_req = DataManager::GetIntValue(TW_FORCE_MD5_CHECK_VAR);
...
wipe_data(int confirm) {
	...
	ui_set_background(BACKGROUND_ICON_WIPE_CHOOSE);
        if (title_headers == NULL) {
            char* headers[] = { (char*) "Confirm wipe of all user data?",
...
main(int argc, char **argv) {
    ...
    if (status != INSTALL_SUCCESS && ui_text_visible()) { // We only want to show menu if error && visible
        ...
        DataManager::LoadValues("/sdcard/TWRP/.twrps");

        // Update some of the main data
        update_tz_environment_variables();
        set_theme(DataManager::GetStrValue(TW_COLOR_THEME_VAR));
